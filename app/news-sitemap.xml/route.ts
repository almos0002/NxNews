...
export async function GET() {
  ...
  let rows: Array<{ slug: string; title_en: string; published_at: string; category: string }> = [];
  try {
    const twoDaysAgo = new Date(Date.now() - 2 * 24 * 60 * 60 * 1000).toISOString();
    const result = await pool.query(
      `SELECT slug, title_en, COALESCE(published_at, created_at) AS published_at, category
       FROM article
