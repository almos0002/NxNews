...
export async function createVideo(data: Omit<Video, "id" | "created_at" | "updated_at">): Promise<Video> {
  const { rows } = await pool.query(
    `INSERT INTO videos (title_en,title_ne,youtube_url,description_en,description_ne,thumbnail,status,author_id)
     VALUES ($1,$2,$3,$4,$5,$6,$7,$8) RETURNING *`,
    [data.title_en, data.title_ne, data.youtube_url, data.description_en, data.description_ne, data.thumbnail, data.status, data.author_id]
  );
  return rows[0];
}
...
export async function updateVideo(id: string, data: Partial<Omit<Video, "id" | "author_id" | "created_at" | "updated_at">>): Promise<Video | null> {
  const { rows } = await pool.query(
    ...
    [id, data.title_en, data.title_ne, data.youtube_url, data.description_en, data.description_ne, data.thumbnail, data.status]
  );
  return rows[0] ?? null;
}
