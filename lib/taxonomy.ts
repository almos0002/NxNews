...
export async function createCategory(data: Omit<Category, "id" | "created_at">): Promise<Category> {
  const { rows } = await pool.query(
    "INSERT INTO categories (name_en,name_ne,slug,color) VALUES ($1,$2,$3,$4) RETURNING *",
    [data.name_en, data.name_ne, data.slug, data.color]
  );
  return rows[0];
}
...
export async function updateCategory(id: string, data: Partial<Omit<Category, "id" | "created_at">>): Promise<Category | null> {
  const { rows } = await pool.query(
    `UPDATE categories SET
       ...
       slug=COALESCE($4,slug), color=COALESCE($5,color)
     WHERE id=$1 RETURNING *`,
    [id, data.name_en, data.name_ne, data.slug, data.color]
  );
  return rows[0] ?? null;
}
